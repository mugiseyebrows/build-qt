...
pushd qt-everywhere-src-6.9.1
    ...
    pushd build
        call ..\configure -prefix C:\Qt\6.9.1\mingw_64 -platform win32-g++ -release -skip qtwebengine -nomake examples -nomake tests -- -DFEATURE_system_zlib=OFF -DFEATURE_sql_mysql=ON -DFEATURE_sql_psql=ON -DPostgreSQL_ROOT=C:/postgresql-14 -DMySQL_ROOT=C:/mysql-8.2.0-winx64
        type config.summary
    popd
popd
where mugideploy > NUL 2>&1 || pip install mugideploy
